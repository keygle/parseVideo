...
def dl_one_file(f):
    ...
    try:
        ...
        log.e('download part file \"' + f['_part_name'] + '\" failed ', add_check_log_prefix=True)
        return False
    except err.PvdlError:
        raise
    except Exception as e:
        log.e('download part file, unknow error ', add_check_log_prefix=True)
...
def _check_local_size(f):
    ...
    if (not 'size' in f) or (f['size'] <= 0):
        log.d('local file size ' + b.byte_to_size(local_size) + ' ', add_check_log_prefix=True)
        log.w('can not check_local_size, no file size info ', add_check_log_prefix=True)
    ...
    if er:
        if (abs(err_u) >= conf.CHECK_ERR_K['local_size_mb']) or (abs(err_k) >= conf.CHECK_ERR_K['local_size']):
            ...
            if not conf.FEATURES['skip_local_larger_file']:
                return False	# not skip
            log.w('enabled feature skip_local_larger_file ', add_check_log_prefix=True)
...
def _check_file_size(f):
    if not conf.FEATURES['check_file_size']:
        log.d('disabled feature check_file_size ', add_check_log_prefix=True)
        return False
    # get file info
    try:
        s = os.stat(f['path'])
    except Exception as e:
        log.e('can not check_file_size \"' + f['_part_name'] + '\", ' + str(e), add_check_log_prefix=True)
...
def _check_file_md5(f):
    ...
    # INFO log
    log.i('checking checksum.md5 for file \"' + f['_part_name'] + '\" ', add_check_log_prefix=True)
    try:	# check file md5
        file_md5 = b.md5sum(f['path'])
    except Exception as e:
        log.e('can not check md5, ' + str(e), add_check_log_prefix=True)
        # TODO print Error info here
        return True	# check failed
    # check md5 match
    if file_md5 != ok_md5:
        log.e('checksum.md5 failed, file ' + file_md5 + ', OK ' + ok_md5 + ' ', add_check_log_prefix=True)
        return True
    log.o('checksum.md5 OK ' + ok_md5 + ' ', add_check_log_prefix=True)
    return False
